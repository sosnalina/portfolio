(function () {
  ...
  // (SCENE0_MARKS.total, computed from SCENE0_SEQUENCE); panel 1 keeps
  // its original 6000ms placeholder span on top of that. Must match
  ...
  // 1's and Gallery 2's own boundaries/parent-JS pairs. (These were
  // still the [0,6000]/12000 placeholders through the scene's initial
  // build — real QA caught the drift: with only this file un-synced,
  // the nav arrows sent seek targets computed from a boundary the
  // iframe's own clock had long since outgrown, so "next" silently
  // seeked to a mid-scene timestamp instead of ever reaching panel 1.)
  var SCENE_BOUNDARIES = [0, 26732];
  var TOTAL_DURATION = 32732;
